...
class FvQuadrilateralGrid(object):
    ...
    def __init__(self, nodes_x, nodes_y):
        self._shape = np.array(np.array(nodes_x).shape) + 1
    ...
    def _initialize_boundary_cells(self, nodes_x, nodes_y):
...
class FvRectilinearGrid(FvQuadrilateralGrid):
    def __init__(self, shape, dims):
        ...
        super(FvRectilinearGrid, self).__init__(*np.meshgrid(x, y, indexing='ij'))
